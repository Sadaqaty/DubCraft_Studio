...
def generate_srt(segments: List[Dict], srt_path: str, speaker_names: Optional[Dict[int, str]] = None) -> None:
    """
    ...
    try:
        with open(srt_path, 'w', encoding='utf-8') as f:
            for i, seg in enumerate(segments, 1):
                ...
                speaker_str = f"[{speaker_names[speaker]}] " if speaker_names and speaker in speaker_names else ""
    ...
    try:
        ...
        for block in blocks:
            ...
            if len(lines) >= 3:
                ...
                segments.append({'start': parse_timestamp(start), 'end': parse_timestamp(end), 'text': text})
    except Exception as e:
